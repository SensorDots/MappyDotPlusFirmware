 ...
 *
 * int status = VL53L1_write_multi(RET_SPAD_EN_0, spad_enables, 36);
 *
 * @endcode
 *
 ...
 *
 * int status = status  = VL53L1_read_multi(DEVICE_ID, buffer, 2)
 *
 * @endcode
 *
 * @param  address - uint8_t device address value
 ...
 *
 * int status = VL53L1_write_word(NVM_CTRL__PULSE_WIDTH_MSB, nvm_ctrl_pulse_width);
 *
 * @endcode
 *
 * @param  address - uint8_t device address value
 ...
 *
 * int status = VL53L1_write_dword(NVM_CTRL__DATAIN_MMM, nvm_data);
 *
 * @endcode
 *
 * @param  address - uint8_t device address value
...
int VL53L1_read_byte(uint8_t address,  uint16_t index, uint8_t  *pdata);
 ...
 * uint16_t timeout = 0;
 *
 * int status = VL53L1_read_word(TIMEOUT_OVERALL_PERIODS_MSB, &timeout);
 *
 * @endcode
 *
 * @param  address - uint8_t device address value
...
int VL53L1_read_word(uint8_t address,  uint16_t index, uint16_t *pdata);
 ...
 * uint32_t range_1 = 0;
 *
 * int status = VL53L1_read_dword(RANGE_1_MMM, &range_1);
 *
 * @endcode
 *
 * @param  address - uint8_t device address value
...
int VL53L1_read_dword(uint8_t address, uint16_t index, uint32_t *pdata);
...
int VL53L1_platform_wait_us(int32_t wait_us);
...
int VL53L1_wait_ms(int32_t wait_ms);
...
int VL53L1_set_gpio(uint8_t  level);
...
int VL53L1_get_gpio(uint8_t *plevel);
...
int VL53L1_get_timer_frequency(int32_t *ptimer_freq_hz);
...
int VL53L1_get_timer_value(int32_t *ptimer_count);
